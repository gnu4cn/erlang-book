...
所谓 *守护序列*，可以是单个的守望，也可以是以分号 (`;`) 分隔的守卫序列。守卫序列 `G1; G2;...;Gn` 在其中至少有一个 -- 即 `G1;G2;...` -- 为 `true` 时，就会求值为 `true`。

而一个 *守卫*，则是一系列以逗号（`,`）分隔的 *守卫表达式*。守卫 `GuardExpr1, GuardExpr2,...,GuardExprN` 在所有守卫表达式 -- 即 `GuardExpr1,GuardExpr2,...,` -- 求值为 `true` 时为 `true`。


有效守卫表达式的集合，是所有有效 Erlang 表达式的一个子集。之所以将守卫表达式限制为 Erlang 表达式的子集，是因为我们希望保证计算守卫表达式，免于一些副作用的影响。守卫表达式是模式匹配的一项扩展，而因为模式匹配没有副作用，我们就不希望守卫的求值有副作用。

此外，守卫（表达式）不能调用用户定义的函数，因为我们希望保证这些守卫（表达式）免于副作用与终止的影响。
