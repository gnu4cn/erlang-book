...
稍后，我们将在 [26.3 节 “并行化顺序代码” 中](../part-v/Ch26-programming_multicore_CPUs.md#并行化序列代码)，展示使用列表综合，编写的一个更简短版本的 `map`，我们将展示咱们如何以 *并行* 方式，计算映射 的所有元素（这将加快我们程序在多核计算机上的运行速度） -- 但这已太超前。既然我们已经知道了 `sum` 和 `map`，咱们就可以用这两个函数，重写 `total` 了：
