...
> ```
>
> 也可以在 Erlang shell 下执行如下命令 `lib_misc:pmap(fun lib_misc:fac/1, [64,128,256]).`。
...
这里，`L` 是 100 个 27 的列表，同时我们要计算列表 `[fib(27), fib(27), ...]` 100 次。(其中 `fib` 是斐波纳契，Fibonacci 函数。）
