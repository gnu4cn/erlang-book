...
Sherlock 的今后开发位于 GitHub 上的 [`joearms/sherlock`](https://github.com/joearms/sherlock)。若咱们有对以下任何问题的好解法，请构造一个 Sherlock 的 fork，并向我（作者）发出拉取请求。Sherlock 这个程序可改进方式很多，正如咱们从下面这个列表中看到的，还有很多事情可完成。


### 找出模组间的相似度

Sherlock 会找出 Erlang 邮件列表中的帖子之间的相似度。请添加一项比较大型 Erlang 模组集合相似度的工具。


### 找出某个模组的历史


除了定义模组间的相似度，我们还可定义两个模组间的 *距离*。一旦我们已找出一组相似模组，就要尝试推导这些模组的演化历史。这些模组是否通过相互剪切粘贴代码，而衍生出来的？当咱们已发现某组相似模组时，他们是否有着共同祖先？


### 分析别的邮件列表中的数据


Sherlock 只能提取 pipermail 存档中的数据。请编写可提取其他常用邮件列表，或论坛程序中邮件的代码。


### Beautiful Soup

Python 有个名为 [Beautiful Soup](https://www.crummy.com/software/BeautifulSoup/)，用于编写网页抓取工具的程序。请实现一个 Erlang 的 Beautiful Soup 版本。使用他收集一些热门邮件列表中的所有邮件。

### 改进文本分析


请改进这个文本分析器。查看其生成的词汇。请编写提取不同文件类型中文本的 Erlang 定制词汇生成器。


### 分面检索


请将更多切面/维度添加到数据，并改进分面检索。


### 构造 Web 界面


请构造一个 Sherlock 的 web 界面。


### 邮件分级

请尝试对邮件分级。邮件的评分，可取决于邮件的撰写者、评论者，以及该邮件引发的后续讨论数量等。


### 绘制结果图形


请构造某次特定查询计算出的相似性关系的图表。在浏览器中以图形显示方式，展示该结果。
...
感谢阅读本书。我（作者）希望咱们享受到这本书。这是一段漫长旅程，咱们以学到了许多新事物。Erlang 的世界观，与别的编程语言截然不同。最大差异在于，我们处理错误的方式，以及我们处理并发的方式。



并发在 Erlang 中是 *天生的*。现实世界确实有着通过消息通信的独立事物。我（作者）是名前物理学家 —— 我们经由接收消息感知世界。光与声的波束承载信息。我们对世界的认知，无非是经由接收消息所获得的知识。


我们没有共享记忆。我有我的记忆，你有你的，同时我不知道你对任何事情的看法。当我想要知道你对某事的看法时，那么我就必须问你问题，然后等待你的答复。

Erlang 使用的编程模型，与现实世界的运作方式极为相似。这使得编程变得简单。许多程序员和企业，都已发现了这点。


那么，接下来呢？请广而告之。请加入 Erlang 邮件列表。参加全球各地举办的众多 Erlang 会议之一。请解出一些有趣程序，并努力让世界成为一个更好的地方。
