...
在下图中，我们可以看到 `mapreduce` 的基本思想。我们有数个会产生 `{Key, Value}` 对的一些流的 *映射* 进程。这些映射进程，会把这些键值对，发送到一个合并这些对的 *化简* 进程，将具有同一个键的那些键值对组合起来。


![`mapreduce` 概念](../images/mapreduce.png)

*警告*：`mapreduce` 上下文中使用的 *map* 一词，与本书其他地方出现的 `map` 函数完全不同。


`mapreduce` 是个 *并行高阶函数*。他由 Google 公司的 Jeffrey Dean 和 Sanjay Ghemawat 提出，据说在 Google 的集群中被日常使用。

我们可以用多种不同方式，及多种不同语义实现 `mapreduce` —— 他实际上更像是一个算法家族，而不是某种特定算法。
...
- `F1(Pid, X)` 为映射函数；
    ...
    `F1` 的工作，是将 `{Key, Value}` 消息的一个流，发送到进程 `Pid`，然后终止。`mapreduce` 将为列表 `L` 中 `X` 的每个值，都生成一个新进程；

- `F2(Key, [Value], Acc0) -> Acc` 为化简函数；

    当全部映射进程都终止时，化简进程便将已合并某个特定键的全部值。`mapreduce` 随后会对其收集到的每个 `{Key, [Value]}` 元组，调用 `F2(Key, [Value], Acc)`。 其中 `Acc` 是个初始值为 `Acc0` 的累加器。 `F2` 会返回一个新累加器。（描述这一点的另一种方式，是说 `F2` 对其收集到的 `{Key, [Value]}` 元组，执行一次 *折叠* 操作。）

    > **译注**：这里的“折叠”, fold，可以看着是 “展开”, flattening 的反义词。

- 其中 `Acc0` 是累加器的初始值，在调用 `F2` 时被用到；

- `L` 是 `X` 值的列表。

    对 `L` 中每个 `X` 值，`F1(Pid, X)` 将被调用。`Pid` 是化简进程的进程标识符；这是由 `mapreduce` 创建的。


`mapreduce` 定义在 `phofs` （*并行高阶函数，parallel higher-order functions* 的缩写）模组中。


```erlang
{{#include ../../projects/ch26-code/phofs.erl}}
```

在继续前，我们将测试 `mapreduce`，以便真正清楚他做了些什么。


我们将编写一个计算这本书随附代码目录中，所有单词频率的小程序。下面是这个程序：


```erlang
{{#include ../../projects/ch26-code/test_mapreduce.erl}}
```
...
当从 1985 年开始以 Erlang 工作时，我们并无并行计算机会变得普遍的概念，也不知道计算机集群可制造于单个芯片上。 Erlang 被设计用于编程容错的多机集群。要达到容错目标，咱们必须有不止一台机器，同时程序必须并发、并行地运行。

当多核心 CPU 到来时，我们发现咱们很多程序只是运行得更快了。这些程序是为并行执行而编写，我们所需的，只是于其上运行他们的并行硬件。

到本书此处，我们已介绍了很多内容。我们已了解了 OTP 框架的一些标准库，与一些主要组件。我们研究了如何构造单机系统，以及如何构建分布式系统中，与多核心上的程序。


在下一章中，我们将使用本书早先开发的许多技术，直接开始开发一个应用。我们将解决夏洛克的最后一个案子。


## 练习

计算的并行化，通常可以显着加快响应时间。我们将以如下练习，展示这点：

1. 在 [获取服务器上的数据](../part-iv/Ch17-programming_with_sockets.md#获取服务器上的数据) 小节中，我们曾编写了个获取 web 页面的程序。请将这个程序修改为发出 `HEAD` 而不是 `GET` 命令。我们可发出 HTTP 的 `HEAD` 命令，测量某个网站的响应时间。在响应 `HEAD` 命令时，只有 web 页面的一些头部返回，而不返回页面正文。请编写一个名为 `web_profiler:ping(URL, Timeout)`，测量某个网站地址 URL 响应时间的函数。这个函数应返回 `{time, T}` 或 `timeout`；

2. 请构造一个名为 `L` 的网站长列表。计算 `lists:map(fun(I) -> web_profiler:ping(URL, Timeout) end, L)` 要用多长时间。这个操作可能需要相当长时间；最坏情况的时间会是 `Timeout x length(L)`；


3. 请使用 `pmap` 而不是 `map`，重复上一测量。现在所有 `HEAD` 的请求都应并行发出。所以，最坏情况的响应时间，仅是 `Timeout`；

4. 请将结果存储在某个数据库中，并构造一个查询该数据库的 web 界面。咱们可将咱们实现这一功能的代码，建立在 [第 24 章，*编程习语*](Ch24-programming_idioms.md) 中，开发的数据库与 Web 服务器代码上；

5. 当咱们以非常长的元素列表调用 `pmap` 时，咱们可能会创建出过多并行进程。请编写一个名为 `pmap(F, L, Max)`，并行地计算列表 `[F(I) || I <- L]`，但受同时运行并行进程数不超过 `Max` 限制的函数；

6. 请编写一个在分布式 Erlang 下，将工作分发到多个 Erlang 节点上的 `pmap` 版本；

7. 请编写一个工作在分布式 Erlang 下，将工作分发到多个 Erlang 节点上，并在节点之间对工作进行负载平衡的 `pmap` 版本。
